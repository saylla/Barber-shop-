// Web Audio Synthesizer for notifications (clean bell / chime / alert)
class SoundService {
  ...
  // Pleasant dual-tone chime for new booking
  playNewBookingChime() {
    try {
  ...
  // Soft tone for status changes
  playStatusChangeChime() {
    try {
      const ctx = this.getAudioContext();
      if (!ctx) return;
